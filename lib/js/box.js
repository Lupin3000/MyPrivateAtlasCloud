    ...
    $.getJSON('./box_all.php', {_: new Date().getTime()}, function(data) {
      ...
      $.each(data, function(i, field) {
        if (typeof field === 'object') {
          ...
          //);
          var markup = '<tr>';
          markup += '<td>' + field.name + '</td>';
          markup += '<td>' + field.description + '</td>';
          markup += '<td>' + field.provider + '</td>';
          markup += '<td>' + btn_info + ' ' + btn_del + '</td></tr>';
          $('#boxTable tbody:last-child').append(markup);
        }
      });
    });
  };
